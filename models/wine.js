...
let db = MongoClient.connectAsync(config.database);

(async function open() {
  try {
    let odb = await Promise.resolve(db);
    console.log("Connected to 'winedb' database");
    try {
      let collection = await odb.collectionAsync('wines', { strict: true });
    } catch (err) {
...
let WineModel = {
  async findById(id) {
    try {
      let odb = await Promise.resolve(db);
      let collection = await odb.collectionAsync('wines');
  ...
  async add(wine) {
    let odb = await Promise.resolve(db);
    let collection = await odb.collection('wines');
